...
`pico` 是一个运行在本地代码仓库中的轻量级 coding agent，专注于以“小步、可验证”的方式协助完成真实工程任务。它通过受约束的工具直接读取项目文件、修改代码、执行命令，并将会话状态保存在本地 `.pico/` 目录中。

相比普通聊天助手，`pico` 更像一个持续驻留在仓库上下文中的命令行搭档。你可以用它排查问题、分析代码结构、修复测试、推进小规模重构，或在当前项目中完成具体的一次性开发任务。
